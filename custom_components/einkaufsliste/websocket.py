...
def ws_item_update(hass, connection, msg):
    fields = _pick(msg, "name", "store_id", "category_id", "quantity", "note")
...
@websocket_api.websocket_command({vol.Required("type"): "einkaufsliste/items/clear_checked"})
@callback
def ws_clear_checked(hass, connection, msg):
    _run(hass, connection, msg, lambda m: {"removed": m.clear_checked()})
...
def ws_cleanup(hass, connection, msg):
    _run(
        ...
        connection,
        msg,
        lambda m: {"removed": len(m.cleanup(force=msg["force"]))},
    )
